...
function styled(base, styles) {
  ...
  if (typeof base === 'function' && base[_constants.TAG_NAME] && base[_constants.STYLES]) {
    return createStyledElement(base[_constants.TAG_NAME], base[_constants.STYLES].concat(styles));
  }
}
...
function createStyledElement(tagName, stylesArray) {
  ...
  element[_constants.TAG_NAME] = tagName;
  element[_constants.STYLES] = stylesArray;
  return element;
}
